...
/*
 ...
 # Copyright (C) 2008 - 2024 magix-cms.com <support@magix-cms.com>
 #
 # OFFICIAL TEAM :
 #
 ...
 * @name plugins_attachfile_public
 */
class plugins_attachfile_public extends plugins_attachfile_db
{
    /**
     ...
     * @return array
     */
    public function setAttachData(array $rawData): array {
        $videos = [];

        if (!empty($rawData)) {
            foreach ($rawData as $key => $value) {

                if (isset($value['id_paf'])) {

                    $videos[$key]['id'] = $value['id_paf'];
                    $videos[$key]['name'] = $value['name_paf'];
                    $videos[$key]['type'] = $value['type_paf'];
    ...
    public function extendProduct(array $data): array {
        return $this->setAttachData($data);
    }
    /**
     * @param array $data
     * @return array
    ...
    public function extendProductData(array $data) : array{
        $extend['newRow'] = ['attachfile' => 'attachfile'];
        $extend['collection'] = 'attachfile';
        if (http_request::isGet('id')) $this->id = form_inputEscape::numeric($_GET['id']);
        if(!empty($data)) {
            $videoCollection = $this->getitems('attach',['id' => $this->id ?? $data['id_product']],'all',false);
